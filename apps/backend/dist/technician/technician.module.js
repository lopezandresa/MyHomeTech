"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TechnicianModule = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const technician_entity_1 = require("./technician.entity");
const technician_service_1 = require("./technician.service");
const technician_controller_1 = require("./technician.controller");
const appliance_entity_1 = require("../appliance/appliance.entity");
const identity_entity_1 = require("../identity/identity.entity");
let TechnicianModule = class TechnicianModule {
};
exports.TechnicianModule = TechnicianModule;
exports.TechnicianModule = TechnicianModule = __decorate([
    (0, common_1.Module)({
        imports: [typeorm_1.TypeOrmModule.forFeature([technician_entity_1.Technician, appliance_entity_1.Appliance, identity_entity_1.Identity])],
        providers: [technician_service_1.TechnicianService],
        controllers: [technician_controller_1.TechnicianController],
    })
], TechnicianModule);
//# sourceMappingURL=technician.module.js.map